...
namespace BMP_example
{
    class Program
    {
        ...
        {
            using (FileStream file = new FileStream("Kvadratai.bmp",
                FileMode.Create, FileAccess.Write))
            {
                ...
                //Suskaičiuojame bmp paveikslėlio eilutės duomenų kiekį baitais 
                //(4 kartotinis) 1000 bit / 32 bit = 31,25 tokiu atveju eilutei reikės 32 baitų po 4 
                int l = 128;
        ...
        static void Rekursija(int n, byte[] t, int kvadr, int pozicija,int l)
        {
            ...
            for (int ii = 0; ii < 5; ii++)
            {
                ...
                switch (ii)
                {
                    //vidurys
                    case 0:
                        Rekursija(n - 1, t, kvadr / 3, pozicija +
                            ((kvadr / 3)) * l + (kvadr / 3) / 8,l);
                        break;
                    //desine virsus
                    case 1:
